...
Router.map(function() {
    ...
    this.route('room', {
        ...
        loginRequired   : 'home',
        data            : function() {
            var roomMessages    = Messages.find({ room : this.params.name }, {sort : {creation_date : 'asc'}});
            return {
        ...
        action          : function() {
            ...
            if(Rooms.find({"name": this.params.name}).count() === 0){
    ...
    this.route('user', {
        ...
        loginRequired   : 'home',
        data            : function() {
            var messages    = Messages.find({$or: [{"user": this.params.username, "receiver": Meteor.user().username},{"receiver": this.params.username, "user": Meteor.user().username}]}, {sort : {creation_date : 'asc'}});
            return {
                messages    : messages,
            }
        }, 
        action          : function() {
            if(Meteor.users.find({"username": this.params.username}).count() === 0){
                this.render("notFound");
                return;
            }
            // Set the Session
...
Router.onBeforeAction(requireLogin, {only: ['user', 'rooms', 'room']});
//Router.onBeforeAction(initClient);