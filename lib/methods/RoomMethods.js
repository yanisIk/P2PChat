...
leaveRoom = function(username, roomName){
	if(roomName){
		...
		Rooms.update({"name": roomName}, {$inc: {"numberOfUsers": -1}});
		if(Rooms.findOne({"name": roomName}).numberOfUsers === 0){
			console.log("Room "+roomUser.room+" is now empty, room will be deleted");
	...
	//Leaving all rooms
		...
		_.each(roomUsers, function(roomUser){
			...
			if(Rooms.findOne({"name": roomUser.room}).numberOfUsers === 0){
				console.log("Room "+roomUser.room+" is now empty, room will be deleted");
				Rooms.remove({"name": roomUser.room});
			}
